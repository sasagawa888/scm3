...
void display_header(void)
{
	...
	CHECK(addch, ' ');
    ESCHOME();
    CHECK(printw, "Edwin %1.2f        File: %s   ", VERSION, fname);
    ESCMOVE(1, COLS - 9);
    if (ed_ins == true) {
	CHECK(addstr, "   insert");
    } else {
...
static const char *functions_data[] = {
    "car", "(car pair) -> object", "Return the first element of the pair",
    "cdr", "(cdr pair) -> object", "Return the rest (tail) of the pair",
    "cons", "(cons a b) -> pair",
	"Construct a pair whose car is a and cdr is b",
    "set-car!", "(set-car! pair obj) -> unspecified",
	"Store obj in the car field of pair",
    "set-cdr!", "(set-cdr! pair obj) -> unspecified",
	"Store obj in the cdr field of pair",
    "list", "(list obj ...) -> list",
	"Return a newly allocated list of the given objects",
    "length", "(length list) -> integer",
	"Return the number of elements in list",
    "append", "(append list ...) -> list", "Append all the given lists",
    "reverse", "(reverse list) -> list",
	"Return a new list with elements in reverse order",
    "memq", "(memq obj list) -> sublist or #f",
	"Find first pair whose car is eq? to obj",
    "memv", "(memv obj list) -> sublist or #f", "Like memq but uses eqv?",
    "member", "(member obj list) -> sublist or #f",
	"Like memq but uses equal?",
    "assq", "(assq obj alist) -> pair or #f",
	"Find first association with key eq? to obj",
    "assv", "(assv obj alist) -> pair or #f", "Like assq but uses eqv?",
    "assoc", "(assoc obj alist) -> pair or #f",
	"Like assq but uses equal?",
    "map", "(map proc list ...) -> list",
	"Apply proc to elements of list(s), returning a list",
    "for-each", "(for-each proc list ...) -> unspecified",
	"Apply proc to each element for side effects",
    "pair?", "(pair? obj) -> boolean", "Return #t if obj is a pair",
    "null?", "(null? obj) -> boolean",
	"Return #t if obj is the empty list",
    "atom?", "(atom? obj) -> boolean", "Return #t if obj is not a pair",
    "symbol?", "(symbol? obj) -> boolean", "Return #t if obj is a symbol",
    "number?", "(number? obj) -> boolean", "Return #t if obj is a number",
    "complex?", "(complex? obj) -> boolean",
	"Return #t if obj is a complex number",
    "real?", "(real? obj) -> boolean", "Return #t if obj is a real number",
    "rational?", "(rational? obj) -> boolean",
	"Return #t if obj is a rational number",
    "integer?", "(integer? obj) -> boolean",
	"Return #t if obj is an integer",
    "exact?", "(exact? obj) -> boolean", "Return #t if obj is exact",
    "inexact?", "(inexact? obj) -> boolean", "Return #t if obj is inexact",
    "=", "(= x1 x2 ...) -> boolean",
	"True if all numbers are numerically equal",
    "<", "(< x1 x2 ...) -> boolean",
	"True if numbers are monotonically increasing",
    ">", "(> x1 x2 ...) -> boolean",
	"True if numbers are monotonically decreasing",
    "<=", "(<= x1 x2 ...) -> boolean", "True if non-decreasing sequence",
    ">=", "(>= x1 x2 ...) -> boolean", "True if non-increasing sequence",
    "+", "(+ x1 ...) -> number", "Sum of all arguments",
    "-", "(- x1 x2 ...) -> number",
	"Subtract all following arguments from the first",
    "*", "(* x1 ...) -> number", "Product of all arguments",
    "/", "(/ x1 x2 ...) -> number",
	"Divide first argument by the product of the rest",
    "abs", "(abs x) -> number", "Return absolute value of x",
    "max", "(max x1 x2 ...) -> number",
	"Return the largest of the arguments",
    "min", "(min x1 x2 ...) -> number",
	"Return the smallest of the arguments",
    "modulo", "(modulo n m) -> integer",
	"Return n mod m with same sign as m",
    "remainder", "(remainder n m) -> integer",
	"Return n remainder m with same sign as n",
    "quotient", "(quotient n m) -> integer",
	"Return integer quotient of n divided by m",
    "expt", "(expt x y) -> number", "Return x raised to the power y",
    "sqrt", "(sqrt x) -> number", "Return square root of x",
    "sin", "(sin x) -> number", "Return sine of x (in radians)",
    "cos", "(cos x) -> number", "Return cosine of x (in radians)",
    "tan", "(tan x) -> number", "Return tangent of x (in radians)",
    "asin", "(asin x) -> number", "Return arcsine of x",
    "acos", "(acos x) -> number", "Return arccosine of x",
    "atan", "(atan x) -> number", "Return arctangent of x",
    "exp", "(exp x) -> number", "Return e raised to the power x",
    "log", "(log x) -> number", "Return natural logarithm of x",
    "not", "(not obj) -> boolean", "Return #t if obj is #f, otherwise #f",
    "boolean?", "(boolean? obj) -> boolean",
	"Return #t if obj is a boolean",
    "char?", "(char? obj) -> boolean", "Return #t if obj is a character",
    "char=?", "(char=? c1 c2) -> boolean",
	"Return #t if characters are equal",
    "char<?", "(char<? c1 c2) -> boolean",
	"Return #t if c1 < c2 alphabetically",
    "string?", "(string? obj) -> boolean", "Return #t if obj is a string",
    "string=?", "(string=? s1 s2) -> boolean",
	"Return #t if strings are equal",
    "string-length", "(string-length str) -> integer",
	"Return number of characters in string",
    "string-ref", "(string-ref str k) -> char",
	"Return character at position k in string",
    "string-set!", "(string-set! str k char) -> unspecified",
	"Store char at index k in string",
    "substring", "(substring str start end) -> string",
	"Return substring from start to end",
    "vector?", "(vector? obj) -> boolean", "Return #t if obj is a vector",
    "vector-length", "(vector-length vec) -> integer",
	"Return length of vector",
    "vector-ref", "(vector-ref vec k) -> object",
	"Return element k of vector",
    "vector-set!", "(vector-set! vec k obj) -> unspecified",
	"Store obj in vector at index k",
    "procedure?", "(procedure? obj) -> boolean",
	"Return #t if obj is a procedure",
    "apply", "(apply proc args) -> object",
	"Apply procedure to list of arguments",
    "eval", "(eval expr env) -> object",
	"Evaluate expression in environment",
    "eq?", "(eq? x y) -> boolean",
	"Return #t if x and y are the same object",
    "eqv?", "(eqv? x y) -> boolean",
	"Return #t if x and y are equivalent objects",
    "equal?", "(equal? x y) -> boolean",
	"Return #t if x and y are structurally equivalent",
    "read", "(read) -> object",
	"Read next expression from current input port",
    "write", "(write obj) -> unspecified",
	"Write object to current output port",
    "display", "(display obj) -> unspecified",
	"Write object without escape characters",
    "newline", "(newline) -> unspecified", "Output a newline character",
    "load", "(load filename) -> unspecified",
